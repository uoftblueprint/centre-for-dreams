  ...
  React.useEffect(() => {
    if (allAbsences) {
      ...
      allAbsences.forEach((absence) => {
        if (new Date(absence.absenceDate).toDateString() == selectedDate?.toDateString()) {
      ...
      <div style={{paddingTop: "32px"}}>
        {
          selectedDate
            ...
            : <div style={{fontSize: "16px", padding: "16px"}}>Pick a Date to View Absentees</div>
        }
      </div>
    </div>
  )
    ...
    absences.length > 0 
      ? <table style={{width: "100%"}}>
          <thead style={{backgroundColor: "#EFF2FB"}}>
            <tr style={{}}>
              <th style={{fontWeight: "normal", textAlign: "left", padding: "12px 16px 12px 32px", width: "200px"}}>S. No.</th>
              <th style={{fontWeight: "normal", textAlign: "left", padding: "12px 32px 12px 16px"}}>Name</th>
            </tr>
          </thead>
          <tbody>
            {absences.map((participantId) => {
              return (
                <tr key={participantId}>
                  <td style={{padding: "12px 16px 12px 32px", width: "200px"}}>{participantId}</td>
                  <td style={{padding: "12px 32px 12px 16px"}}>{`User ${participantId}`}</td>
                </tr>
              )
            })}
          </tbody>
