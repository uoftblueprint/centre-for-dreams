        ...
        <div className={`w-screen h-screen bg-black/80 flex justify-center items-center fixed top-0 left-0 ${open ? "" : "invisible"}`} onSubmit={handleSubmit(onSubmit)}>
            {activity.data && <form className='m-12 w-full min-w-[320px] sm:w-2/3 lg:w-1/2 xl:w-2/5 border-b-2 bg-white rounded-lg max-sm:text-sm font-semibold p-6 sm:p-12 flex gap-3 flex-col'>
                <div className='flex flex-col gap-2'>
                    ...
                    <input required {...register("name", { required: true })} id='name' name='name' onChange={handleInputChange} type='text' className='bg-slate-200 rounded-md p-3 font-normal text-sm focus:bg-gray-100 focus:outline-none hover:bg-slate-200/70' />
                </div>
                    ...
                    <div className='flex gap-2 w-full'>
                        <div className='w-full flex flex-col gap-2'>
                            ...
                            <input onChange={(e) => {
                                ...
                                required name='startTime' id='startTime' type='time' className='bg-slate-200 rounded-md p-3 font-normal text-sm focus:bg-gray-100 focus:outline-none w-full hover:bg-slate-200/70 cursor-pointer' />
                        </div>
                        <div className='w-full flex flex-col gap-2'>
