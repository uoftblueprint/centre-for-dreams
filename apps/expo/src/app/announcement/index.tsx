...
function AnnouncementPage() {
  const { id, createdAt, title, contents } = useLocalSearchParams();
  ...
  return (
    <SafeAreaView className="h-full bg-white pb-[75px]">
      <Stack.Screen options={{ title: "Announcement", headerShown: false }} />
      <ScrollView>
        ...
        <View className="bg-p-99 px-4 py-4">
          <Text className="font-headline-sm leading-8">{title}</Text>
        </View>
        ...
        <View className="flex flex-row gap-3 px-4 py-3">
          ...
          <View>
            ...
            <Text className="font-body-lg">{formatTime(createdDate)}</Text>
          </View>
        </View>

        <View>
          <Text className="font-body-lg px-4">{contents}</Text>
        </View>
