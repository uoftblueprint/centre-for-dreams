...
const Jeff = () => {
  ...
  return (
    <div className="m-4 block max-w-sm border-4 border-black p-4">
      ...
      <p className="mb-2 text-black">{jeff_info?.introduction}</p>
      ...
      <div className="flex flex-row items-center gap-4">
        <button
          className="mt-4 border-4 border-black bg-black px-10 py-2 font-bold text-white hover:bg-white hover:text-black focus:outline-none active:bg-black"
          onClick={() => mutation.mutate()}
        >
          Upvote
        </button>
        <p className="pt-2 font-bold text-black">
          Upvotes: {jeff_info?.upvotes}
        </p>
      </div>
    </div>
  );
